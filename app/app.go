...
func (app *Basecoin) Query(reqQuery abci.RequestQuery) (resQuery abci.ResponseQuery) {
	fmt.Println("querysafd")
	fmt.Println(reqQuery.String())
	fmt.Println(hex.DecodeString(string(reqQuery.Data[:])))

	if reqQuery.Path == "sequence" {
			fmt.Println("sequence")
			queryData,err := hex.DecodeString(string(reqQuery.Data[:]))
			if err != nil {
	...
	if reqQuery.Path == "balance" {
		...
		//address := utilQueryStruct.Address
		acc := app.state.GetAccount(queryData)
		resQuery.Index = acc.Balance[0].Amount
		return resQuery
	}
