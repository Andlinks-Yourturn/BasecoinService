...
func validateInputAdvanced(acc *types.Account, signBytes []byte, in types.TxInput) (res abci.Result) {
	...
	//edit by ligang
	//fmt.Println("in validateInputAdvance ")
	//fmt.Println(data.Bytes(acc.PubKey.Address()).String())
	//messageChan := make(chan string, 1)
	//if util.IsExitInMap(data.Bytes(acc.PubKey.Address()).String()) {
	//	fmt.Println("in validateInputAdvance +1")
	//	go func() {
	//		util.CheckAuthority(&messageChan)
	//	}()
	//	 <- messageChan
	//	fmt.Println("i get the money")
	//}
